...
class Flecsi(CMakePackage):
    ...
    depends_on('legion@ctrl-rep +shared +mpi build_type=Debug +hdf5', when='backend=legion build_type=Debug +hdf5')
    depends_on('legion@ctrl-rep +shared +mpi build_type=Debug', when='backend=legion build_type=Debug ~hdf5')
    ...
    depends_on('hpx@1.3.0 cxxstd=14 build_type=Debug', when='backend=hpx build_type=Debug')
