...
class Flecsi(CMakePackage):
    ...
    depends_on('legion@ctrl-rep-5 +shared +mpi build_type=Debug +hdf5', when='backend=legion +debug_backend +hdf5')
    depends_on('legion@ctrl-rep-5 +shared +mpi build_type=Debug', when='backend=legion +debug_backend ~hdf5')
    depends_on('legion@ctrl-rep-5 +shared +mpi build_type=Release +hdf5', when='backend=legion ~debug_backend +hdf5')
