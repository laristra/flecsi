...
fill_task(const Legion::Task * task,
  ...
  int count = 0;
  for(LegionRuntime::Arrays::GenericPointInRectIterator<1> pir(rect); pir;
      pir++) {
    count++;
...
// Define a Legion task to register.
int
internal_task_example_1(const Legion::Task * task,
  const std::vector<Legion::PhysicalRegion> & regions,
  Legion::Context context,
  Legion::Runtime * runtime) {
...
driver(int argc, char ** argv) {
  ...
  LegionRuntime::Arrays::Rect<1> elem_rect(LegionRuntime::Arrays::Point<1>(0),
    LegionRuntime::Arrays::Point<1>(num_elmts + num_ghost - 1));
  IndexSpace is =
    runtime->create_index_space(context, Domain::from_rect<1>(elem_rect));
  ...
  Legion::TaskLauncher task_launcher(
    context_t::instance().task_id(key_0), Legion::TaskArgument(nullptr, 0));
  ...
  IndexPartition ex_ip, sh_ip, gh_ip;
  {
    ...
    gh_ip = runtime->create_index_partition(
      context, is, color_domain, gh_coloring, false /*disjoint*/);
  }
  ...
  index_launcher.add_region_requirement(RegionRequirement(
    sh_lp, 0 /*projection ID*/, READ_WRITE, EXCLUSIVE, stencil_lr));
  index_launcher.add_field(1, FID_VAL);
  index_launcher.add_region_requirement(RegionRequirement(
    gh_lp, 0 /*projection ID*/, READ_ONLY, EXCLUSIVE, stencil_lr));
