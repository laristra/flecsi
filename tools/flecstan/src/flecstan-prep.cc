...
Preprocessor::MacroExpands(const clang::Token & token,
  ...
  const clang::MacroArgs * const arguments) /* override */ {
  ...
  if(expect.empty()) {
    ...
    expect =
      nested_macros.insert(std::make_pair(name, std::queue<std::string>{}))
        .first->second;
  }
  else {
  ...
  pos2macro[std::make_tuple(
              mi.location.file, mi.location.line, mi.location.column)]
    .push_back(mi);
...
const MacroInvocation *
Preprocessor::invocation(const clang::SourceLocation & loc) const {
