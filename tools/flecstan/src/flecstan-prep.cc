...
void Preprocessor::MacroExpands(
   ...
   const clang::MacroArgs *const macroargs
) /* override */ {
   ...
   */
   ...
   if (expect.empty()) {
      ...
      expect = nested_macros.insert(
         std::make_pair(original = macname, std::queue<std::string>{})
      ).first->second;
   } else {
...
const MacroCall *Preprocessor::findcall(
