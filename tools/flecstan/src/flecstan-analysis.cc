...
analyze_flecsi_get_handle(const flecstan::Yaml & yaml) {
  ...
  */
  ...
  for(auto gh : yaml.flecsi_get_handle.matched) {
    ...
    // efficiency will be super important with these macros.
    ...
    if(!pair_was_registered) {
      ...
      // Continue to the next [flecsi_get_handle] call; we can't check
      // that this one is consistent with its registration, because the
      // error right here is that we didn't find a matching registration.
      continue;
    }
...
same_scope(const flecsi_base & a, const flecsi_base & b) {
  ...
  for(std::size_t i = a.scope.size(); i--;)
    if(a.scope[i] == unnamed_namespace || b.scope[i] == unnamed_namespace ||
       a.scope[i] != b.scope[i])
      return false;
...
task_reg_dup(const std::multimap<std::string, treg> & regs) {
  ...
  //    hash (primary)
  //    unit (secondary)
  sort(tmp.begin(), tmp.end(), [](const treg & a, const treg & b) -> bool {
    return a.hash < b.hash || (a.hash == b.hash && (a.unit < b.unit));
  });
  ...
  for(std::size_t i = 0, j = 0; i < size; i = j) {
    ...
    while(
      ++j < size && tmp[j].hash == tmp[i].hash && tmp[j].unit == tmp[i].unit) {
      // begin diagnostic
      if(str == "")
      ...
      // for summary
      summary_task_reg_dup += "(" + uflcs(tmp[j], false) +
                              ") dups "
                              "(" +
                              uflcs(tmp[i], false) + ")\n";
    }
  ...
  for(std::size_t i = 0, j = 0; i < size; i = j) {
    ...
    while(++j < size && tmp[j].hash == tmp[i].hash) {
...
task_reg_without_exe(const std::multimap<std::string, treg> & regs,
  ...
  for(auto reg : regs)
    if(exes.find(reg.first) == exes.end()) {
      status =
        warning("The task registered with hash \"" + reg.first +
                "\" here:\n   " + uflcs(reg.second) +
                "\n"
...
task_exe_without_reg(const std::multimap<std::string, treg> & regs,
  ...
  for(auto exe : exes)
    if(regs.find(exe.first) == regs.end()) {
      status = error(
        "The task executed with hash \"" + exe.first + "\" here:\n   " +
...
function_reg_dup(
  ...
  sort(tmp.begin(), tmp.end(),
    [](const flecsi_register_function & a,
      const flecsi_register_function & b) -> bool {
      return a.hash < b.hash || (a.hash == b.hash && (a.unit < b.unit));
    });
  ...
  for(std::size_t i = 0, j = 0; i < size; i = j) {
    ...
    while(
      ...
      // for summary
      summary_function_reg_dup += "(" + uflcs(tmp[j], false) +
                                  ") dups "
                                  "(" +
                                  uflcs(tmp[i], false) + ")\n";
    }
