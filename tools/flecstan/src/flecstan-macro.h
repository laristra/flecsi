...
#define flecstan_expand(apply_macro,sym) \
   \
   apply_macro ( flecsi_register_program ) sym \
   apply_macro ( flecsi_register_top_level_driver ) sym \
   \
   apply_macro ( flecsi_register_global_object ) sym \
   apply_macro ( flecsi_set_global_object ) sym \
   apply_macro ( flecsi_initialize_global_object ) sym \
   apply_macro ( flecsi_get_global_object ) sym \
   \
   apply_macro ( flecsi_register_task_simple ) sym \
   apply_macro ( flecsi_register_task ) sym \
   apply_macro ( flecsi_register_mpi_task_simple ) sym \
   apply_macro ( flecsi_register_mpi_task ) sym \
   \
   apply_macro ( flecsi_color ) sym \
   apply_macro ( flecsi_colors ) sym \
   \
   apply_macro ( flecsi_execute_task_simple ) sym \
   ...
   \
   apply_macro ( flecsi_register_function ) sym \
   apply_macro ( flecsi_execute_function ) sym \
   apply_macro ( flecsi_function_handle ) sym \
   apply_macro ( flecsi_define_function_type ) sym \
   \
   apply_macro ( flecsi_register_data_client ) sym \
   apply_macro ( flecsi_register_field ) sym \
   apply_macro ( flecsi_register_global ) sym \
   apply_macro ( flecsi_register_color ) sym \
   \
   apply_macro ( flecsi_get_handle ) sym \
   apply_macro ( flecsi_get_client_handle ) sym \
   apply_macro ( flecsi_get_handles ) sym \
   apply_macro ( flecsi_get_handles_all ) sym \
   \
   apply_macro ( flecsi_get_global ) sym \
   apply_macro ( flecsi_get_color ) sym \
   apply_macro ( flecsi_get_mutator ) sym
...
flecstan_class(flecsi_register_program)
   std::string program;
flecstan_class_done

flecstan_maptraits(flecsi_register_program)
   flecstan_map(program);
flecstan_maptraits_done



// flecsi_register_top_level_driver ( driver )
flecstan_class(flecsi_register_top_level_driver)
   std::string driver;
flecstan_class_done

flecstan_maptraits(flecsi_register_top_level_driver)
   flecstan_map(driver);
flecstan_maptraits_done
...
flecstan_class(flecsi_register_global_object)
   std::string index;
   std::string nspace;
   std::string type;
flecstan_class_done

flecstan_maptraits(flecsi_register_global_object)
   flecstan_map(index);
   flecstan_map(nspace);
   flecstan_map(type);
flecstan_maptraits_done
...
flecstan_class(flecsi_set_global_object)
   std::string index;
   std::string nspace;
   std::string type;
   std::string obj;
...
flecstan_maptraits(flecsi_set_global_object)
   flecstan_map(index);
   flecstan_map(nspace);
   flecstan_map(type);
   flecstan_map(obj);
...
flecstan_class(flecsi_initialize_global_object)
   std::string index;
   std::string nspace;
   std::string type;
...
flecstan_maptraits(flecsi_initialize_global_object)
   flecstan_map(index);
   flecstan_map(nspace);
   flecstan_map(type);
...
flecstan_class(flecsi_get_global_object)
   std::string index;
   std::string nspace;
   std::string type;
flecstan_class_done

flecstan_maptraits(flecsi_get_global_object)
   flecstan_map(index);
   flecstan_map(nspace);
   flecstan_map(type);
flecstan_maptraits_done
...
flecstan_class(flecsi_register_task_simple)
   std::string task;
...
flecstan_maptraits(flecsi_register_task_simple)
   flecstan_map(task);
...
flecstan_class(flecsi_register_task)
   std::string task;
   std::string nspace;
   std::string processor;
   std::string launch;
...
flecstan_maptraits(flecsi_register_task)
   flecstan_map(task);
   flecstan_map(nspace);
   flecstan_map(processor);
   flecstan_map(launch);
...
flecstan_class(flecsi_register_mpi_task_simple)
   std::string task;
   std::string hash;
flecstan_class_done

flecstan_maptraits(flecsi_register_mpi_task_simple)
   flecstan_map(task);
   flecstan_map(hash);
flecstan_maptraits_done
...
flecstan_class(flecsi_register_mpi_task)
   std::string task;
   std::string nspace;
   std::string hash;
flecstan_class_done

flecstan_maptraits(flecsi_register_mpi_task)
   flecstan_map(task);
   flecstan_map(nspace);
   flecstan_map(hash);
flecstan_maptraits_done
...
// flecsi_color ( )
flecstan_class(flecsi_color)
flecstan_class_done

flecstan_maptraits(flecsi_color)
flecstan_maptraits_done



// flecsi_colors ( )
flecstan_class(flecsi_colors)
flecstan_class_done

flecstan_maptraits(flecsi_colors)
flecstan_maptraits_done
...
flecstan_class(flecsi_execute_task_simple)
   std::string task;
   std::string launch;
   std::vector<VarArgTypeValue> varargs;
   std::string hash;
...
flecstan_maptraits(flecsi_execute_task_simple)
   flecstan_map(task);
   flecstan_map(launch);
   flecstan_map(varargs);
   flecstan_map(hash);
...
flecstan_class(flecsi_execute_task)
   std::string task;
   std::string nspace;
   std::string launch;
   std::vector<VarArgTypeValue> varargs;
...
flecstan_maptraits(flecsi_execute_task)
   flecstan_map(task);
   flecstan_map(nspace);
   flecstan_map(launch);
   flecstan_map(varargs);
...
flecstan_class(flecsi_execute_mpi_task_simple)
   std::string task;
   std::vector<VarArgTypeValue> varargs;
   std::string hash;
flecstan_class_done

flecstan_maptraits(flecsi_execute_mpi_task_simple)
   flecstan_map(task);
   flecstan_map(varargs);
   flecstan_map(hash);
flecstan_maptraits_done
...
flecstan_class(flecsi_execute_mpi_task)
   std::string task;
   std::string nspace;
   std::vector<VarArgTypeValue> varargs;
   std::string hash;
...
flecstan_class(flecsi_register_reduction_operation)
   std::string type;
   std::string datatype;
flecstan_class_done

flecstan_maptraits(flecsi_register_reduction_operation)
   flecstan_map(type);
   flecstan_map(datatype);
flecstan_maptraits_done
...
flecstan_class(flecsi_register_function)
   std::string func;
   std::string nspace;
   std::string hash;
flecstan_class_done

flecstan_maptraits(flecsi_register_function)
   flecstan_map(func);
   flecstan_map(nspace);
   flecstan_map(hash);
flecstan_maptraits_done
...
flecstan_class(flecsi_execute_function)
   std::string handle;
   std::vector<VarArgTypeValue> varargs;
flecstan_class_done

flecstan_maptraits(flecsi_execute_function)
   flecstan_map(handle);
   flecstan_map(varargs);
flecstan_maptraits_done
...
flecstan_class(flecsi_function_handle)
   std::string func;
   std::string nspace;
   std::string hash;
flecstan_class_done

flecstan_maptraits(flecsi_function_handle)
   flecstan_map(func);
   flecstan_map(nspace);
   flecstan_map(hash);
flecstan_maptraits_done
...
flecstan_class(flecsi_define_function_type)
   std::string func;
   std::string return_type;
   std::vector<VarArgType> varargs;
flecstan_class_done

flecstan_maptraits(flecsi_define_function_type)
   flecstan_map(func);
...
flecstan_class(flecsi_register_data_client)
...
flecstan_maptraits(flecsi_register_data_client)
   flecstan_map(client_type);
...
// The macro in question calls the register_field() that's defined (at the time
// of this writing) in field.h. That function has "size_t INDEX_SPACE = 0" as
// its last template argument. So, the macro's ##__VA_ARGS__, which is the last
// template argument it sends to register_field(), is an (optional) index space.
flecstan_class(flecsi_register_field)
   std::string   client_type;
   std::string   nspace;
   std::string   name;
...
flecstan_maptraits(flecsi_register_field)
   flecstan_map(client_type);
   flecstan_map(nspace);
   flecstan_map(name);
...
flecstan_class(flecsi_register_global)
   std::string   nspace;
   std::string   name;
   std::string   data_type;
   std::uint32_t versions;
...
flecstan_maptraits(flecsi_register_global)
   flecstan_map(nspace);
   flecstan_map(name);
   flecstan_map(data_type);
   flecstan_map(versions);
...
flecstan_class(flecsi_register_color)
   std::string   nspace;
   std::string   name;
   std::string   data_type;
   std::uint32_t versions;
...
flecstan_maptraits(flecsi_register_color)
   flecstan_map(nspace);
   flecstan_map(name);
   flecstan_map(data_type);
   flecstan_map(versions);
...
flecstan_class(flecsi_get_handle)
...
flecstan_maptraits(flecsi_get_handle)
   flecstan_map(client_handle);
   flecstan_map(nspace);
   flecstan_map(name);
   flecstan_map(data_type);
   flecstan_map(storage_class);
...
flecstan_class(flecsi_get_client_handle)
   std::string   client_type;
   std::string   nspace;
   std::string   name;
flecstan_class_done

flecstan_maptraits(flecsi_get_client_handle)
   flecstan_map(client_type);
   flecstan_map(nspace);
   flecstan_map(name);
flecstan_maptraits_done
...
flecstan_class(flecsi_get_handles)
   std::string   client;
   std::string   nspace;
   std::string   data_type;
   std::string   storage_class;
...
flecstan_maptraits(flecsi_get_handles)
   flecstan_map(client);
   flecstan_map(nspace);
   flecstan_map(data_type);
   flecstan_map(storage_class);
...
flecstan_class(flecsi_get_handles_all)
   std::string   client;
   std::string   data_type;
   std::string   storage_class;
   std::string   version;
...
flecstan_maptraits(flecsi_get_handles_all)
   flecstan_map(client);
   flecstan_map(data_type);
   flecstan_map(storage_class);
   flecstan_map(version);
...
flecstan_class(flecsi_get_global)
   std::string   nspace;
   std::string   name;
   std::string   data_type;
   std::uint32_t version;
...
flecstan_maptraits(flecsi_get_global)
   flecstan_map(nspace);
   flecstan_map(name);
   flecstan_map(data_type);
   flecstan_map(version);
...
flecstan_class(flecsi_get_color)
   std::string   nspace;
   std::string   name;
   std::string   data_type;
   std::uint32_t version;
...
flecstan_maptraits(flecsi_get_color)
   flecstan_map(nspace);
   flecstan_map(name);
   flecstan_map(data_type);
   flecstan_map(version);
...
flecstan_class(flecsi_get_mutator)
   std::string   client_handle;
   std::string   nspace;
   std::string   name;
   std::string   data_type;
...
flecstan_maptraits(flecsi_get_mutator)
   flecstan_map(client_handle);
   flecstan_map(nspace);
   flecstan_map(name);
   flecstan_map(data_type);
