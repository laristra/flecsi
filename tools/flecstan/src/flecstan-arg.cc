...
std::string fixdir(const std::string &in)
{
   // begin, end
   int b = 0;
   int e = in.size();

   while (b < e && in[b  ] == ' ') b++;  // remove beginning spaces
   while (e > b && in[e-1] == ' ') e--;  // remove ending spaces
   while (e > b && in[e-1] == '/') e--;  // remove ending /s
...
// version
bool option_version(const std::string &opt)
   { flecstan_setnfind("-version", "--version"); }

// help
bool option_help(const std::string &opt)
   { flecstan_setnfind("-help", "--help"); }

// quiet
inline bool option_quiet  (const std::string &opt)
   { flecstan_setnfind("-quiet",   "--quiet"  ); }

// verbose
inline bool option_verbose(const std::string &opt)
   { flecstan_setnfind("-verbose", "--verbose"); }

// short
inline bool option_short(const std::string &opt)
   { flecstan_setnfind("-short", "--short"); }

// long
inline bool option_long (const std::string &opt)
   { flecstan_setnfind("-long",  "--long" ); }

// print
inline bool option_print(const std::string &opt)
   { flecstan_setnfind("-print", "--print"); }


// debug
inline bool option_debug(const std::string &opt)
   { flecstan_setnfind("-debug", "--debug"); }
inline bool option_no_debug(const std::string &opt)
   { flecstan_setnfind("-no-debug", "--no-debug"); }


// heading
inline bool option_heading(const std::string &opt)
{
   flecstan_setnfind(
      "-heading",  "--heading",
      "-headings", "--headings",
...
inline bool option_no_heading(const std::string &opt)
{
   flecstan_setnfind(
...
// various
flecstan_toggle(file)
flecstan_toggle(report)
flecstan_toggle(note)
flecstan_toggle(warning)
...
inline bool option_no_ccline(const std::string &opt)
{
   flecstan_setnfind(
      "-no-cc-line",  "--no-cc-line",
      "-no-cc-lines", "--no-cc-lines"
   );
}
...
inline bool option_no_analysis(const std::string &opt)
   { flecstan_setnfind("-no-analysis", "--no-analysis"); }

inline bool option_summary(const std::string &opt)
   { flecstan_setnfind("-summary", "--summary"); }
inline bool option_no_summary(const std::string &opt)
...
inline bool option_clang(const std::string &opt)
{
   flecstan_setnfind(
      "-clang",   "--clang",
      "-clang++", "--clang++"
...
inline bool option_flags(const std::string &opt)
{
   flecstan_setnfind(
      "-flag",  "--flag",
      "-flags", "--flags"
...
inline bool option_dir(const std::string &opt)
{
   flecstan_setnfind(
      "-dir",       "--dir",
      "-directory", "--directory"
...
inline bool option_json(const std::string &opt)
{
   flecstan_setnfind(
      "-json", "--json"
   );
...
inline bool option_cc(const std::string &opt)
{
   flecstan_setnfind(
      "-cc",  "--cc",
      "-cpp", "--cpp",
...
inline bool option_yaml(const std::string &opt)
{
   flecstan_setnfind(
      "-yaml", "--yaml"
   );
...
inline bool option_any(const std::string &opt)
{
   return
      ...
      option_debug    (opt) || option_no_debug    (opt) ||
      option_heading  (opt) || option_no_heading  (opt) ||
      option_file     (opt) || option_no_file     (opt) ||
      option_report   (opt) || option_no_report   (opt) ||
      option_note     (opt) || option_no_note     (opt) ||
...
inline bool endsin_cc(const std::string &str)
{
   return
      endsin(str, ".cc" ) ||
      endsin(str, ".cpp") ||
...
inline bool endsin_yaml(const std::string &str)
{
...
exit_status_t process_clang(
   ...
   if (int(++i) == argc ||
       option_any (argv[i]) ||
       endsin_json(argv[i]) || endsin_cc(argv[i]) || endsin_yaml(argv[i])
   ) {
      i--;
      return error("The " + opt + " option (clang++ compiler) "
                   "expects an argument.\n"
                   "Example: " + opt + " /usr/bin/clang++");
   }
...
exit_status_t process_dir(
   ...
   if (int(++i) == argc ||
      ...
      i--;
      return error("The " + opt + " option (JSON/C++ file directory) "
                   "expects an argument.\n"
                   "Example: " + opt + " /some/directory/\n"
   ...
   note(std::string(com.dir.set() ? "Changing " : "Setting ") +
       "directory to " + quote(dir) + ".");
...
exit_status_t one_json(
   ...
   if (found) {
      ...
      if (!json)
         note("File name " + quote(full) + " doesn't end "
   ...
   while (true) {
      if (int(++i) == argc || // no more arguments, or...
         ...
         return arguments
            ...
            ? status
            // Had no arguments
            : error("The " + opt + " option (JSON compilation database(s)) "
                    "expects one or more arguments.\n"
                    "Example: " + opt + " foo.json bar.json");
      }
   ...
   while (true) {
      if (int(++i) == argc || // no more arguments, or...
         ...
         return arguments
            ...
            ? status
            // Had no arguments
            : error("The " + opt + " option (C++ file(s)) "
                    "expects one or more arguments.\n"
                    "Example: " + opt + " one.cc two.cpp three.cxx four.C");
      }
...
{
   // version, help
   if (option_version(opt)) { print_version(); }
   if (option_help   (opt)) { print_help   (); }
   ...
   // short, long
   else if (option_short(opt)) { short_form = true;  }
   else if (option_long (opt)) { short_form = false; }
   ...
   else if (option_debug      (opt)) { emit_debug   = true;  }
   else if (option_no_debug   (opt)) { emit_debug   = false; }
   else if (option_heading    (opt)) { emit_heading = true;  }
   else if (option_no_heading (opt)) { emit_heading = false; }
   else if (option_file       (opt)) { emit_file    = true;  }
   else if (option_no_file    (opt)) { emit_file    = false; }
   else if (option_report     (opt)) { emit_report  = true;  }
   else if (option_no_report  (opt)) { emit_report  = false; }
   else if (option_note       (opt)) { emit_note    = true;  }
   else if (option_no_note    (opt)) { emit_note    = false; }
   else if (option_warning    (opt)) { emit_warning = true;  }
   else if (option_no_warning (opt)) { emit_warning = false; }
   ...
   else if (option_no_summary (opt)) { emit_section_summary  = false; }

   else if (option_column     (opt)) { emit_column = true;  }
   else if (option_no_column  (opt)) { emit_column = false; }
   else if (option_ccline     (opt)) { emit_ccline = true;  }
   else if (option_no_ccline  (opt)) { emit_ccline = false; }
...
exit_status_t arguments(
   ...
   if (!(com.database.size() || // no given JSON
      ...
      return error(
        "No input file(s) specified, and default (" +
         default_json_file + ") not found.\n"
