  ...
  ~Consumer()
   ...
   void HandleTranslationUnit(clang::ASTContext &context) override
   {
      ...
      if (!ci.hasSema()) {
         status = std::max(
            status,
            error("Could not get the Semantic Analyzer "
  ...
  ~Action()
   ...
   std::unique_ptr<clang::ASTConsumer> CreateASTConsumer(
      ...
      if (!ci.hasPreprocessor()) {
         status = std::max(
            status,
            error("Could not get the Preprocessor "
                  "for this Compiler Instance."));
         return nullptr;
      }
...
std::unique_ptr<clang::tooling::CompilationDatabase> read_json(
   const std::pair<
      std::string,
      bool
   > &pair
...
bool try_file(const clang::tooling::CompileCommand &cc)
{
   // directory, file
   ...
   for (std::size_t j=0, c=0, n=0;  n < size && status != exit_fatal;  ++n) {
      ...
      if (run != 0) {
         ...
         oss << "Clang's ClangTool::run() "
                "reports a nonzero return value: " << run << ".";
         if (run == 2)
...
exit_status_t yamlout(bool &written, bool &already)
{
   debug("yamlout()");
...
int main(const int argc, const char *const *const argv)
{
   ...
   while (arg+1 < std::size_t(argc)) {
      if (flecstan::option_version(argv[arg+1])) {
         ...
         if (!already) flecstan::print_version();
         already = true;
      } else if (flecstan::option_help(argv[arg+1])) {
         static bool already = false;
         if (!already) flecstan::print_help();
         already = true;
      } else
         break;
      // no arguments other than version or help?
      if (++arg+1 == std::size_t(argc)) return exit_clean;
   }
