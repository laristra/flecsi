...
class structured_topology_u : public structured_topology_base_t
{
  ...
  static_assert(
    std::is_convertible<decltype(POLICY::num_dimensions), size_t>::value,
...
public:
  ...
  structured_topology_u & operator=(const structured_topology_u &) = delete;

  // Allow move operations
  structured_topology_u(structured_topology_u && o) = default;
  structured_topology_u & operator=(structured_topology_u && o) = default;

  // Copy constructor
  structured_topology_u(const structured_topology_u & m)
    : storage_(m.storage()) {}
