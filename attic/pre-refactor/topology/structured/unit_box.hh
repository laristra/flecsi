...
constexpr inline size_t power(size_t base, size_t exponent) {
...
//! the lower/upper bounds can be defined w.r.t to the outer box. The strides are
//! the strides of the outer box. 
//!
//!   *--------------------*
//!   |                    |
//!   |                    |
...
template<size_t MESH_DIMENSION, size_t ENTITY_DIMENSION, bool BND_TAGGED=false>
class unit_box_u 
{
public: 
 ...
 *************************************************************************/                             
 ...
 void initialize(std::vector<size_t> &lowerbnds, 
		 std::vector<size_t> &upperbnds,
                 ...
                 bool &is_bndtagged, 
                 std::vector<bool> &bnd_tags, bool primary)
 {
 ...
 *****************************************************************************/                             
 ...
 {
   return ((index >= 0) &&
           (index <= local_upper_bound<BOX_ID,DIM>()));
 ...
 *****************************************************************************/                             
 ...
 //--------------------------------------------------------------------------//
  void global_box_indices_from_global_box_offset(const id_t& global_box_id,
       const id_t& global_box_offset, id_array_t &indices)
  {
    id_array_t str;
...
//--------------------------------------------------------------------------//
 ...
 //! @param global_offset Global offset of an entity
 //--------------------------------------------------------------------------//
  void global_box_indices_from_global_offset(const id_t& global_offset, id_array_t &indices)
  {
   ...
   id_t& box_id, id_array_t &indices)
  {
    id_t offset;
    global_box_offset_from_global_offset(global_offset, box_id, offset);
    global_box_indices_from_global_box_offset(box_id, offset, indices);
 ...
 //--------------------------------------------------------------------------//
  auto global_offset_from_global_box_indices(const id_t& global_box_id,
       const id_array_t& global_box_indices)
  {
     id_t offset = global_box_offset_from_global_box_indices(global_box_id,
      global_box_indices);
 ...
 void local_box_indices_from_local_box_offset(const id_t local_box_id,
       const id_t& local_box_offset, id_array_t &indices)
  {
    id_array_t str;
...
//--------------------------------------------------------------------------//
 ...
 //--------------------------------------------------------------------------//
  void local_box_indices_from_local_offset(const id_t& local_offset, id_array_t &indices)
  {
    id_t box_id = 0, offset;
 ...
 //--------------------------------------------------------------------------//
  auto local_offset_from_local_box_indices(const id_t& local_box_id,
       const id_array_t& local_box_indices)
  {
     id_t offset = local_box_offset_from_local_box_indices(local_box_id,
      local_box_indices);
...
//--------------------------------------------------------------------------//
 ...
 //--------------------------------------------------------------------------//
  void global_box_indices_from_local_box_indices(const id_t& local_box_id,
       const id_array_t& local_box_indices, id_array_t &indices)
  {
    //id_array_t id;
    for (size_t i=0; i <MESH_DIMENSION; ++i)
      indices[i] = local_box_indices[i] + lowerbnds_[MESH_DIMENSION*local_box_id+i];
   //return id;
...
//--------------------------------------------------------------------------//
 ...
 //--------------------------------------------------------------------------//
  void local_box_indices_from_global_box_indices(const id_t& global_box_id,
       const id_array_t& global_box_indices, id_array_t &indices)
  {
    //id_array_t id;
    for (size_t i=0; i <MESH_DIMENSION; ++i)
      indices[i] = global_box_indices[i] - lowerbnds_[MESH_DIMENSION*global_box_id+i];
   //return id;
...
//--------------------------------------------------------------------------//
 ...
 //--------------------------------------------------------------------------//
  ...
  {
    id_array_t id;
    global_box_indices_from_local_box_offset(local_box_id, local_box_offset, id);
...
//--------------------------------------------------------------------------//
 ...
 //--------------------------------------------------------------------------//
  ...
  {
    id_array_t id;
    local_box_indices_from_global_box_offset(global_box_id, global_box_offset, id);
...
//--------------------------------------------------------------------------//
 ...
 //--------------------------------------------------------------------------//
  ...
  {
    id_array_t id;
    local_box_indices_from_global_box_indices(global_box_id, global_box_indices, id);
    return local_box_offset_from_local_box_indices(global_box_id, id);
...
//--------------------------------------------------------------------------//
 ...
 //--------------------------------------------------------------------------//
  ...
  {
    id_array_t id;
    global_box_indices_from_local_box_indices(local_box_id, local_box_indices, id);
    return global_box_offset_from_global_box_indices(local_box_id, id);
...
//--------------------------------------------------------------------------//
 ...
 //--------------------------------------------------------------------------//
  ...
  {
   id_array_t id;
   global_box_indices_from_global_box_offset(global_box_id, global_box_offset, id);
...
//--------------------------------------------------------------------------//
 ...
 //! @param local_offset  local offset of the entity
 //--------------------------------------------------------------------------//
  void global_box_indices_from_local_offset(const id_t& local_offset, id_array_t &indices)
  {
     id_t box_id;
     id_array_t id;
     local_box_indices_from_local_offset(local_offset, box_id, id);
...
//--------------------------------------------------------------------------//
 ...
 //! @param global_offset Global offset of an entity
 //--------------------------------------------------------------------------//
  void local_box_indices_from_global_offset(const id_t& global_offset, id_array_t &indices)
  {
    id_t box_id;
    id_array_t id;
    global_box_indices_from_global_offset(global_offset, box_id, id);
 ...
 * Iterators  
 *************************************************************************/                             
 template<class ENTITY, size_t BOX_ID=NUM_BOXES+1, bool BOUNDARY_TAG=BND_TAGGED>
 auto begin() 
 {
   ...
   return iterator_u<ENTITY, 
		     unit_box_u<MESH_DIMENSION, ENTITY::dimension, BND_TAGGED>, 
                     BOX_ID, BOUNDARY_TAG>(this, start, true); 
 ...
 auto end() 
 {
   ...
   id_t end = (BOX_ID==NUM_BOXES+1) ? local_size() : local_range_up<BOX_ID>(); 
  
   return iterator_u<ENTITY, 
                     unit_box_u<MESH_DIMENSION, ENTITY::dimension, BND_TAGGED>, 
                     BOX_ID, BOUNDARY_TAG>(this, end, false); 
 }; 

 template<class ENTITY, class UB, size_t BOX_ID=NUM_BOXES+1, bool BOUNDARY_TAG=BND_TAGGED>
 class iterator_u 
 {
  public:
 ...
 template<class UB, size_t BOX_ID=NUM_BOXES+1, bool BOUNDARY_TAG=BND_TAGGED>
 class iterator_ 
 {
  public:
   ...
   {
     //std::cout<<"Before: lid = "<<current_local_<<", gid = "<<current_global_<<std::endl;  
	...
	current_local_ = ubox_->find_valid_id(current_local_);
     ...
     //std::cout<<"After: lid = "<<current_local_<<", gid = "<<current_global_<<std::endl;  
   }; 
   ...
   while (!valid) {
    ...
    for (size_t b = MESH_DIMENSION; b < bid_list.size(); ++b) {
     ...
     for (size_t d = 0; d < MESH_DIMENSION; ++d) {
       ...
       std::cout<<" b = "<<b<<": d = "<<d<<": lbnd = "<<lbnd<<": ubnd = "<<ubnd<<": b2d = "<<bid2dir_map[bid_list[b]]<<std::endl; 
...
private:
 ...
 // Utility methods 
 auto compute_offset_from_indices( id_array_t &strides, 
				   id_array_t &indices)
 {
   id_t value = 0;
