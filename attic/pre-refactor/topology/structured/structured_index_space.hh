...
struct structured_index_space_u {
  ...
  void initialize(box_coloring_t &colored_is) 
  {
    ...
    for (size_t i=0; i <nboxes; i++) {
     for (size_t j=0; j <MESH_DIMENSION; j++){
       lbnds[MESH_DIMENSION*i+j] = ebox[i].domain.lowerbnd[j] - obox[i].lowerbnd[j];
       ubnds[MESH_DIMENSION*i+j] = ebox[i].domain.upperbnd[j] - obox[i].lowerbnd[j];
       strs[MESH_DIMENSION*i+j] = obox[i].upperbnd[j] - obox[i].lowerbnd[j] + 1; 
     }
    ...
    for (size_t k=0; k <sbox[0].size(); ++k) {
     for (size_t i=0; i <nboxes; ++i) {
      for (size_t j=0; j <MESH_DIMENSION; ++j) { 
        lbnds[MESH_DIMENSION*i+j] = sbox[i][k].domain.lowerbnd[j] - obox[i].lowerbnd[j];
        ubnds[MESH_DIMENSION*i+j] = sbox[i][k].domain.upperbnd[j] - obox[i].lowerbnd[j];
        strs[MESH_DIMENSION*i+j] = obox[i].upperbnd[j] - obox[i].lowerbnd[j] + 1; 
      } 
    ...
    for (size_t k=0; k <gbox[0].size(); ++k) {
     for (size_t i=0; i <nboxes; ++i) {
      for (size_t j=0; j <MESH_DIMENSION; ++j) {
        lbnds[MESH_DIMENSION*i+j] = gbox[i][k].domain.lowerbnd[j] - obox[i].lowerbnd[j];
        ubnds[MESH_DIMENSION*i+j] = gbox[i][k].domain.upperbnd[j] - obox[i].lowerbnd[j];
        strs[MESH_DIMENSION*i+j] = obox[i].upperbnd[j] - obox[i].lowerbnd[j] + 1; 
      } 
    ...
    for (size_t k=0; k <dbox[0].size(); ++k) {
     for (size_t i=0; i <nboxes; ++i) {
      for (size_t j=0; j <MESH_DIMENSION; ++j) {
        lbnds[MESH_DIMENSION*i+j] = dbox[i][k].domain.lowerbnd[j] - obox[i].lowerbnd[j];
        ubnds[MESH_DIMENSION*i+j] = dbox[i][k].domain.upperbnd[j] - obox[i].lowerbnd[j];
        strs[MESH_DIMENSION*i+j] = obox[i].upperbnd[j] - obox[i].lowerbnd[j] + 1; 
      } 
  ...
  void convert_bounds_to_local(box_t& outer, box_t& inner, 
