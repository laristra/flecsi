...
color(dcrs const & naive, idx_t colors, MPI_Comm comm = MPI_COMM_WORLD) {
  ...
  flog_assert(naive.global_colors() == size_t(size),
    "invalid naive coloring! naive.colors(" << colors
    << ") must equal comm size(" << size << ")");
...
distribute(dcrs const & naive, size_t colors,
  std::vector<size_t> const & index_colors, MPI_Comm comm = MPI_COMM_WORLD) {
  int size, rank;
  MPI_Group group;
  ...
  int result = MPI_Alltoall(&send_cnts[0], 1, mpi_size_t, 
    &recv_cnts[0], 1, mpi_size_t, comm);
  ...
  for(size_t r{0}; r<size_t(size); ++r) {
    if(recv_cnts[r]) {
      ...
      requests.push_back({});
      MPI_Irecv(&rbuffers[r][0], recv_cnts[r]*2, mpi_size_t,
        r, 0, comm, &requests[requests.size() - 1]);
    } // if
  } // for
  ...
  for(size_t r{0}; r<size_t(size); ++r) {
    if(send_cnts[r]) {
      ...
      MPI_Send(&sbuffers[r][0], send_cnts[r]*2, mpi_size_t,
        r, 0, comm);
    } // if
  } // for
