...
template<std::size_t DIMENSION,
  ...
  std::size_t TO_DIMENSION = DIMENSION,
  std::size_t THRU_DIMENSION = DIMENSION-1>
void make_dcrs_have_connectivity(
  ...
  for(size_t cell( dcrs.distribution[rank] ); cell < dcrs.distribution[rank+1]; ++cell)
  {
    ...
    for(auto vertex : md.entities(FROM_DIMENSION, THRU_DIMENSION, cell)) {
      ...
      for ( auto other : md.entities(THRU_DIMENSION, TO_DIMENSION, vertex) ) {
        ...
        if ( FROM_DIMENSION == TO_DIMENSION ){
          if ( cell != other ) this_cell2other.push_back( other );
        }
        else {
...
auto alltoallv(
  ...
  for (size_t rank = 0; rank < num_ranks; ++rank) {
    ...
    if (count > 0) {
      ...
      auto ret = MPI_Irecv(buf, count, mpi_recv_t, rank, tag, comm, &my_request);
  ...
  for(size_t rank = 0; rank < num_ranks; ++rank) {
    ...
    if (count > 0) {
      ...
      requests.resize(requests.size() + 1);
      auto & my_request = requests.back();
      auto ret = MPI_Isend(buf, count, mpi_send_t, rank, tag, comm, &my_request);
      if ( ret != MPI_SUCCESS ) return ret;
    }
  }
...
void subdivide( size_t nelem, size_t npart, Vector<T> & dist ) {
...
void make_dcrs_distributed(
  ...
  MPI_Allgather( &num_cells, 1, mpi_size_t, dcrs.distribution.data()+1,
  ...
  for ( size_t ic=0; ic<num_cells; ++ic ) {
    ...
    for ( auto i = cells2vertex.offsets[ic]; i<cells2vertex.offsets[ic+1]; ++i )
    {
  ...
  std::vector<size_t> sendbuf;
  {
    ...
    for (size_t r=0; r<size && it != vertex2cell.end(); ++it) {
      while (it->first >= vert_dist[r+1]) r++;
  ...
  auto ret = MPI_Alltoall(sendcounts.data(), 1, mpi_size_t, recvcounts.data(), 1, mpi_size_t, MPI_COMM_WORLD);
  ...
  ret = alltoallv(sendbuf, sendcounts, senddispls, recvbuf,
      recvcounts, recvdispls, MPI_COMM_WORLD );
  if ( ret != MPI_SUCCESS ) clog_error( "Error communicating new vertices" );
  ...
  for ( size_t ic=0; ic<num_cells; ++ic ) {
    ...
    for ( auto i = start; i<end; ++i ) {
      ...
      for (auto other : vertex2cell.at(vertex)) {
        ...
        if ( other != cell ) cell_counts[other] += 1;
      }
...
void migrate(
  ...
  MPI_Allgather( &num_elements, 1, mpi_size_t, dcrs.distribution.data()+1,
    1, mpi_size_t, MPI_COMM_WORLD);
...
void color_entities(
  ...
  for ( const auto & pair : entities2rank ) {
    ...
    for ( auto rank : ranks ) {
      if ( rank != comm_rank ) sendcounts[rank] += n;
    }
  } // vert
  ...
  for ( const auto & pair : entities2rank ) {
    ...
    for ( auto rank : ranks ) {
      ...
      if ( rank != comm_rank ) {
        ...
        for ( auto r : owner.ghost_ranks ) sendbuf[i++] = r;
        sendcounts[rank] += n;
      }
    }
  } // vert
  ...
  for ( const auto pair : entities2rank ) {
    ...
    if ( owner.rank == comm_rank ) {
      ...
      auto it = entities.shared.emplace( global_id, owner.rank, owner.local_id,
          owner.ghost_ranks );
