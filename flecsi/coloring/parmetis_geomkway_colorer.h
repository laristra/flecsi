...
struct parmetis_geomkway_colorer_t : public colorer_t {
   ...
   */
  parmetis_geomkway_colorer_t & operator=(
    const parmetis_geomkway_colorer_t &) = delete;
  ...
  std::vector<size_t> new_color(size_t num_parts,
    ...
    if(has_ents) {
      ...
      int result = ParMETIS_V3_PartGeomKway(&vtxdist[0], &xadj[0], &adjncy[0],
        nullptr, nullptr, &wgtflag, &numflag, &ndims_, xyz_.data(), &ncon,
        &npart, &tpwgts[0], ubvec.data(), options, &edgecut, &part[0], &comm);
      if(result != METIS_OK)
