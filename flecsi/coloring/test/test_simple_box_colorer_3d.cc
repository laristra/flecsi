...
TEST(simple_colorer, simpletest3d) {
  ...
  if(rank == 26) {
    ...
    cout << "Rank-" << rank << "::Exclusive-Box:LBND  = { "
         << col.exclusive.box.lowerbnd[0] << ", "
         << col.exclusive.box.lowerbnd[1] << ", "
         << col.exclusive.box.lowerbnd[2] << " } " << endl;

    cout << "Rank-" << rank << "::Exclusive-Box:UBND  = { "
         << col.exclusive.box.upperbnd[0] << ", "
    ...
    for(auto s : col.shared) {
      cout << "Rank-" << rank << "::Shared-Boxes:LBND  = { "
           << s.box.lowerbnd[0] << ", " << s.box.lowerbnd[1] << ", "
           << s.box.lowerbnd[2] << " } " << endl;

      cout << "Rank-" << rank << "::Shared-Boxes:UBND  = { "
           << s.box.upperbnd[0] << ", " << s.box.upperbnd[1] << ", "
    ...
    for(auto g : col.ghost) {
      cout << "Rank-" << rank << "::Ghost-Boxes:LBND  = { " << g.box.lowerbnd[0]
           << ", " << g.box.lowerbnd[1] << ", " << g.box.lowerbnd[2] << " } "
           << endl;

      cout << "Rank-" << rank << "::Ghost-Boxes:UBND  = { " << g.box.upperbnd[0]
           << ", " << g.box.upperbnd[1] << ", " << g.box.upperbnd[2] << " } "
           << endl;
    ...
    for(auto d : col.domain_halo) {
      cout << "Rank-" << rank << "::DomainHalo-Boxes:LBND  = { "
           << d.lowerbnd[0] << ", " << d.lowerbnd[1] << ", " << d.lowerbnd[2]
           << " } " << endl;

      cout << "Rank-" << rank << "::DomainHalo-Boxes:UBND  = { "
           << d.upperbnd[0] << ", " << d.upperbnd[1] << ", " << d.upperbnd[2]
           << " } " << endl;
    }
  }
}
