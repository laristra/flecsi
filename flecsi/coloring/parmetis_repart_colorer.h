...
struct parmetis_repart_colorer_t : public colorer_t {
   ...
   */
  parmetis_repart_colorer_t & operator=(
    const parmetis_repart_colorer_t &) = delete;
  ...
  std::vector<size_t> new_color(size_t num_parts,
    ...
    if(num_ents) {
      ...
      if(rank == 0) {
        if(num_parts != size) {
          std::cout << "WARNING: repartitiioner will not create new partitions"
                    << std::endl;
        }
      }
      ...
      int defval =
        (size == num_parts) ? std::numeric_limits<idx_t>::max() : world_rank;
