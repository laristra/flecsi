...
float  *a_foo(int, float, double) { return new float (1.2f); }
float  *b_foo(int, float, double) { return new float (3.4f); }
double *bar_1(double, double)     { return new double(5.6 ); }
double *bar_2(double, double)     { return new double(8.9 ); }

// These handlers do something slightly more interesting. Well, less boring.
float  *add_ifd(int i, float f, double d) { return new float (i+f+float(d)); }
double *add_dd (double d1, double d2)     { return new double(d1+d2); }
