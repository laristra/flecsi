...
TEST(hash, all) {
  ...
  EXPECT_EQ(flecsi::utils::string_hash_u("abc", 123UL, 0, 0), 123);
  EXPECT_EQ(flecsi::utils::string_hash_u("abc", 456UL, 1, 1), 456);
  EXPECT_EQ(flecsi::utils::string_hash_u("abc", 789UL, 2, 2), 789);

  // the third argument < the second argument; return something more complex
  EXPECT_EQ(flecsi::utils::string_hash_u("abc", 123, 0, 1), 26);
  EXPECT_EQ(flecsi::utils::string_hash_u("abc", 456, 1, 2), 25544);
  EXPECT_EQ(flecsi::utils::string_hash_u("abc", 789, 1, 3), 6512917);

  // no, because 4 is greater than the string's length
  // EXPECT_EQ(flecsi::utils::hash_u("abc", 123, 0, 4), );

  // no, because we'd keep ++'ing 1 until we get 0 :-/
  // EXPECT_EQ(flecsi::utils::hash_u("abc", 123, 1, 0), );
