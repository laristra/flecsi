...
typename std::enable_if<I == sizeof...(Tp), void>::type increment(
    std::tuple<Tp...> & t)
{
}
...
typename std::enable_if<(sizeof...(Ts) > 0), bool>::type not_equal_tuples(
    const std::tuple<T1, Ts...> & t1, const std::tuple<T1, Ts...> & t2)
{
  return (std::get<0>(t1) != std::get<0>(t2)) ||
...
template <typename T1, typename... Ts>
typename std::enable_if<(sizeof...(Ts) > 0), bool>::type equal_tuples(
    const std::tuple<T1, Ts...> & t1, const std::tuple<T1, Ts...> & t2)
{
  return (std::get<0>(t1) == std::get<0>(t2)) &&
...
class zipper
{
 public:
  ...
  zipper(T1 & a, Ts &... b)
      : begin_(a.begin(), (b.begin())...), end_(a.end(), (b.end())...){};

  zipper(const zipper<T1, Ts...> & a) : begin_(a.begin_), end_(a.end_){};
