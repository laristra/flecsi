...
/**
  ...
  */
  static void mpi_qsort(std::vector<T> &ents, int nents) {
    ...
    std::sort(
      ents.begin(), ents.end(), [](auto &left, auto &right) {
...
/**
  ...
  */
  static void generate_splitters_samples_(
    ...
    std::vector<T> &ents, const int64_t& nents) {
    ...
    MPI_Gatherv(&keys_sample[0], nsample * sizeof(std::pair<key_t, int64_t>),
    ...
    MPI_Bcast(&splitters[0],
      (size - 1 + 2) * sizeof(std::pair<key_t, int64_t>), MPI_BYTE,
      0, MPI_COMM_WORLD);
  } // generate_splitters_samples_

  static void mpi_alltoallv_p2p_(
    ...
    std::vector<type_t> &recvbuffer) 
  {
    ...
    for (int i = 0; i < size; ++i) {
      if (recvcount[i] != 0) {
        ...
        MPI_Recv(start + recvoffsets[i], recvcount[i], MPI_BYTE, i, 
