...
struct sph_ntree_t : topo::specialization<topo::ntree, sph_ntree_t> {
  ...
  static coloring color(const std::string& name, std::vector<ent_t>& ents) {
    ...
    for(int i = 0 ; i < size; ++i){
      ...
      if(rank == 0)
        std::cout<<rank<<": "<<c.offset_[i].first<<" - "<<c.offset_[i].second<<std::endl; 
    }
...
const field<double>::definition<sph_ntree_t, sph_ntree_t::base::entities> entity_field;
auto pressure = entity_field(sph_ntree);

int init(sph_ntree_t::accessor<wo> t, const std::vector<sph_ntree_t::ent_t>& ents) {
  UNIT {
