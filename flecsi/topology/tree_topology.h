...
public:
  ...
  //-----------------------------------------------------------------//
  void update_branches(const element_t epsilon = element_t(0)) {
    // Recursive version 
  ...
  //! Go through all the branches in a DFS order
  //-----------------------------------------------------------------//
  void update_branches_local(const element_t epsilon = element_t(0)) {
    // Recursive version 
    std::function<void(branch_t*)> traverse;
    traverse = [&epsilon,&traverse,this](branch_t* b){
      element_t mass = element_t(0); 
      ...
      b->set_bmax(bmax);  
      if(b->sub_entities() > 0)
        std::cout << "Adding branch: mass=" << b->getMass() 
                  << " coord="  << b->get_coordinates() 
                  << " sub_entities =" << b->sub_entities() 
                  << " min=" << b->bmin() 
                  << " max=" << b->bmax() <<std::endl;
    };
    traverse(root());
  ...
  spheroid.
  //-----------------------------------------------------------------//
  subentity_space_t find_in_radius_b(
      const point_t& center,
      element_t radius) {
  ...
  //! box.
  //-----------------------------------------------------------------//
  subentity_space_t find_in_box_b(const point_t & min, const point_t & max) {
    subentity_space_t ents;
    ents.set_master(entities_);
...
public:
  ...
  tree_entity() : branch_id_(branch_id_t::null()), locality_(NONLOCAL) {}
...
public:
  ...
  void set_coordinates(point_t coordinates) {
  ...
  void set_bmin(point_t bmin) {
    bmin_ = bmin;
  }
 
  void set_bmax(point_t bmax) {
    bmax_ = bmax;
  }
  ...
  point_t get_coordinates() const {
    return coordinates_;
  }

  uint64_t sub_entities() const {
    return sub_entities_;
  }

  void set_sub_entities(uint64_t sub_entities) {
