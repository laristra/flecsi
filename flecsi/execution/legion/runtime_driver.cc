...
)
{
  ...
  for(auto idx_space: coloring_info) {
    ...
    char buf[80];
    sprintf(buf, "expanded index space %ld", idx_space.first);
    ...
    sprintf(buf, "expanded field space %ld", idx_space.first);
    ...
    sprintf(buf, "expanded logical region %ld", idx_space.first);
    ...
    sprintf(buf, "color partitioing %ld", idx_space.first);
