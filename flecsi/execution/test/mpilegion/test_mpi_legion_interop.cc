...
// user includes
#include "flecsi/execution/test/mpilegion/interop_init.h"
...
/* ------------------------------------------------------------------------ */
 ...
 {
  InteropS::getInstance().helper.connect_with_mpi(ctx, runtime);

  std::cout<<"inside TLT:after connect_with_mpi"<<std::endl;

  InteropS::getInstance().helper.wait_on_mpi(ctx, runtime);
  ArgumentMap arg_map;
  IndexLauncher helloworld_launcher(
       HELLOWORLD_TASK_ID,
       Domain::from_rect<1>(
              InteropS::getInstance().helper.all_processes_),
       TaskArgument(0, 0),
       arg_map);
  ...
  //handoff to MPI
  InteropS::getInstance().helper.handoff_to_mpi(ctx, runtime);
  ...
  //check array storage 
   ...
   index_partition_t ip = InteropS::getInstance().helper.data_storage_[0];
   double A= InteropS::getInstance().helper.data_storage_[1];
