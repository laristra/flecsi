...
)
{
  ...
  for (int master_color=0; master_color < num_ranks; ++master_color) {
      ...
      for (std::set<ptr_t>::iterator it=shared_coloring[master_color].points.begin();
              it!=shared_coloring[master_color].points.end(); ++it) {
          const ptr_t ptr = *it;
          for (int slave_color = 0; slave_color < num_ranks; ++slave_color)
              if (ghost_coloring[slave_color].points.count(ptr)) {
