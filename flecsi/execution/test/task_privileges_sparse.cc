...
flecsi_register_field(mesh_t, hydro, pressure, size_t, sparse, 1, index_spaces::cells);
...
task_A(mesh<ro> mesh, field<rw, rw, na> h) {
  ...
  for(auto c : mesh.cells(exclusive)) {
    for(auto entry : h.entries(c)) {
     CINCH_CAPTURE() << "task A exclusive cell "<<rank<<" "
                     << c->id()<<" "<<c->gid()<<" "
                     << c->index()[0] << " " << c->index()[1]<<" "
                     <<entry<<" "<< h(c, entry)
                     << std::endl;
  ...
  for(auto c : mesh.cells(shared)) {
    for(auto entry : h.entries(c)) { 
     CINCH_CAPTURE() << "task A shared cell "<<rank<<" "
                     << c->id()<<" "<<c->gid()<<" "
                     << c->index()[0] << " " << c->index()[1]<<" "
                     <<entry<<" "<< h(c, entry)
                     << std::endl;
...
task_B(mesh<ro> mesh, field<rw, ro, ro> h) {
  ...
  for(auto c : mesh.cells(exclusive)) {
    for(auto entry : h.entries(c)) {
     CINCH_CAPTURE() << "task B exclusive cell "<<rank<<" "
                     << c->id()<<" "<<c->gid()<<" "
                     << c->index()[0] << " " << c->index()[1]<<" "
                     <<entry<<" "<< h(c, entry)
                     << std::endl;
  ...
  for(auto c : mesh.cells(shared)) {
    for(auto entry : h.entries(c)) { 
     CINCH_CAPTURE() << "task B shared cell "<<rank<<" "
                     << c->id()<<" "<<c->gid()<<" "
                     << c->index()[0] << " " << c->index()[1]<<" "
                     <<entry<<" "<< h(c, entry)
                     << std::endl;
  ...
  for(auto c : mesh.cells(ghost)) {
    for(auto entry : h.entries(c)) {
     CINCH_CAPTURE() << "task B ghost cell "<<rank<<" "
                     << c->id()<<" "<<c->gid()<<" "
                     << c->index()[0] << " " << c->index()[1]<<" "
                     <<entry<<" "<< h(c, entry)
                     << std::endl;
...
task_C(mesh<ro> mesh, field<rw, ro, wo> h) {
  ...
  for(auto c : mesh.cells(exclusive)) {
    for(auto entry : h.entries(c)) {
     CINCH_CAPTURE() << "task C exclusive cell "<<rank<<" "
                     << c->id()<<" "<<c->gid()<<" "
                     << c->index()[0] << " " << c->index()[1]<<" "
                     <<entry<<" "<< h(c, entry)
                     << std::endl;
  ...
  for(auto c : mesh.cells(shared)) {
    for(auto entry : h.entries(c)) { 
     CINCH_CAPTURE() << "task C shared cell "<<rank<<" "
                     << c->id()<<" "<<c->gid()<<" "
                     << c->index()[0] << " " << c->index()[1]<<" "
                     <<entry<<" "<< h(c, entry)
                     << std::endl;
  ...
  for(auto c : mesh.cells(ghost)) {
    for(auto entry : h.entries(c)) {
     CINCH_CAPTURE() << "task C ghost cell "<<rank<<" "
                     << c->id()<<" "<<c->gid()<<" "
                     << c->index()[0] << " " << c->index()[1]<<" "
                     <<entry<<" "<< h(c, entry)
                     << std::endl;
...
task_D(mesh<ro> mesh, field<rw, rw, rw> h) {
  ...
  for(auto c : mesh.cells(exclusive)) {
    for(auto entry : h.entries(c)) {
     CINCH_CAPTURE() << "task D exclusive cell "<<rank<<" "
                     << c->id()<<" "<<c->gid()<<" "
                     << c->index()[0] << " " << c->index()[1]<<" "
                     <<entry<<" "<< h(c, entry)
                     << std::endl;
  ...
  for(auto c : mesh.cells(shared)) {
    for(auto entry : h.entries(c)) { 
     CINCH_CAPTURE() << "task D shared cell "<<rank<<" "
                     << c->id()<<" "<<c->gid()<<" "
                     << c->index()[0] << " " << c->index()[1]<<" "
                     <<entry<<" "<< h(c, entry)
                     << std::endl;
  ...
  for(auto c : mesh.cells(ghost)) {
    for(auto entry : h.entries(c)) {
     CINCH_CAPTURE() << "task D ghost cell "<<rank<<" "
                     << c->id()<<" "<<c->gid()<<" "
                     << c->index()[0] << " " << c->index()[1]<<" "
                     <<entry<<" "<< h(c, entry)
                     << std::endl;
...
task_E(mesh<ro> mesh, field<ro, ro, ro> h) {
  ...
  for(auto c : mesh.cells(exclusive)) {
    for(auto entry : h.entries(c)) {
     CINCH_CAPTURE() << "task E exclusive cell "<<rank<<" "
                     << c->id()<<" "<<c->gid()<<" "
                     << c->index()[0] << " " << c->index()[1]<<" "
                     <<entry<<" "<< h(c, entry)
                     << std::endl;
  ...
  for(auto c : mesh.cells(shared)) {
    for(auto entry : h.entries(c)) { 
     CINCH_CAPTURE() << "task E shared cell "<<rank<<" "
                     << c->id()<<" "<<c->gid()<<" "
                     << c->index()[0] << " " << c->index()[1]<<" "
                     <<entry<<" "<< h(c, entry)
                     << std::endl;
  ...
  for(auto c : mesh.cells(ghost)) {
    for(auto entry : h.entries(c)) {
     CINCH_CAPTURE() << "task E ghost cell "<<rank<<" "
                     << c->id()<<" "<<c->gid()<<" "
                     << c->index()[0] << " " << c->index()[1]<<" "
                     <<entry<<" "<< h(c, entry)
                     << std::endl;
...
task_F(mesh<ro> mesh, field<rw, rw, ro> h) {
  ...
  for(auto c : mesh.cells(exclusive)) {
    for(auto entry : h.entries(c)) {
     CINCH_CAPTURE() << "task F exclusive cell "<<rank<<" "
                     << c->id()<<" "<<c->gid()<<" "
                     << c->index()[0] << " " << c->index()[1]<<" "
                     <<entry<<" "<< h(c, entry)
                     << std::endl;
  ...
  for(auto c : mesh.cells(shared)) {
    for(auto entry : h.entries(c)) { 
     CINCH_CAPTURE() << "task F shared cell "<<rank<<" "
                     << c->id()<<" "<<c->gid()<<" "
                     << c->index()[0] << " " << c->index()[1]<<" "
                     <<entry<<" "<< h(c, entry)
                     << std::endl;
  ...
  for(auto c : mesh.cells(ghost)) {
    for(auto entry : h.entries(c)) {
     CINCH_CAPTURE() << "task F ghost cell "<<rank<<" "
                     << c->id()<<" "<<c->gid()<<" "
                     << c->index()[0] << " " << c->index()[1]<<" "
                     <<entry<<" "<< h(c, entry)
                     << std::endl;
