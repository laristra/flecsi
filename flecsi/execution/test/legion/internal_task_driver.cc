...
// Define a Legion task to register.
int
internal_task_example_2(const Legion::Task * task,
  const std::vector<Legion::PhysicalRegion> & regions,
  Legion::Context context,
  Legion::Runtime * runtime) {
...
driver(int argc, char ** argv) {
  ...
  // Lookup the task ids.
  auto tid_1 = context_t::instance()
                 .task_id<flecsi_internal_task_key(internal_task_example_1)>();
  auto tid_2 = context_t::instance()
                 .task_id<flecsi_internal_task_key(internal_task_example_2)>();
  ...
  Legion::TaskLauncher launcher(
    context_t::instance().task_id(key_1), Legion::TaskArgument(0, 0));
