...
struct data_hash_key_t : public std::pair<size_t, size_t> {
  ...
  ///
  const size_t namespace_hash() const {
...
struct data_hash_t {
  ...
  static data_hash_key_t make_key(size_t namespace_hash, size_t name_hash) {
    return std::make_pair(namespace_hash, name_hash);
  } // make_key

  std::size_t operator()(const data_hash_key_t & key) const {
    return ((key.namespace_hash() << 32) ^ key.name_hash());
  } // operator ()

  bool operator()(const data_hash_key_t & a, const data_hash_key_t & b) const {
