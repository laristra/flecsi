...
.. sidebar:: Top-level Action
  ...
  The top-level action is a C++ function created and set by the user to
  tell FleCSI what it should do when the start control point is invoked.
...
* **start** |br|
  ...
  bulk of any simulation that is performed using FleCSI. Before invoking
  *start*, the user must set a top-level action that FleCSI will
  execute.
...
.. include:: sources/2-program_options.rst

Example 3: FLOG (FleCSI Logging Utility)
****************************************

.. include:: sources/3-flog.rst
